...
unmatched_records = pd.read_csv(data_dir + "species lists/gbif_matched/unmatched_records_usagekey_uid.csv").rename(columns={"usagekey":"usageKey"})
unmatched_records["New"] = True
unmatched_records["Date"] = f"{today.year}-{today.month:02d}-{today.day:02d}"

# Turn usageKey into an integer, then a string
unmatched_records["usageKey"] = unmatched_records["usageKey"].astype("Int64").astype(str)
